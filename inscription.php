...
if (isset($_POST['inscription']) && $_POST['inscription'] == 'Inscription') {
    ...
    if ((isset($_POST['login']) && !empty($_POST['login'])) && (isset($_POST['pass']) && !empty($_POST['pass'])) && (isset($_POST['pass_confirm']) && !empty($_POST['pass_confirm']))  && (isset($_POST['email']) && !empty($_POST['email']))) {
        ...
        if ($_POST['pass'] != $_POST['pass_confirm']) {
            ...
            $sql = 'SELECT count(*) FROM identifiant_membre WHERE membre_pseudo="'.mysqli_escape_string($base,$_POST['login']).'" or membre_email="'.mysqli_escape_string($base,$_POST['login']).'" ';
            $req = mysqli_query($base,$sql) or die('Erreur SQL !<br />'.$sql.'<br />'.mysqli_error($base));
            $data = mysqli_fetch_array($req);
            if ($data[0] == 0) {
                $sql = 'INSERT INTO identifiant_membre VALUES("", "'.mysqli_escape_string($base,$_POST['login']).'", "'.mysqli_escape_string($base,md5($_POST['pass'])).'")';
