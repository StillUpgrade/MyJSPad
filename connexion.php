...
if (isset($_POST['connexion']) && $_POST['connexion'] == 'Connexion') {
    if ((isset($_POST['login']) && !empty($_POST['login'])) && (isset($_POST['pass']) && !empty($_POST['pass']))) {
        ...
        $sql = 'SELECT count(*) FROM membre_pseudo WHERE (membre_pseudo="'.mysqli_escape_string($base,$_POST['login']).'" or membre_email ="'.mysqli_escape_string($base,$_POST['email']).'" ) AND membre_mdp="'.mysqli_escape_string($base,md5($_POST['pass'])).'"';
